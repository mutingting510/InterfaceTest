...
class Interface():
    ...
    def runTest(self):
        ExcelFile = xlrd.open_workbook(r'D:\monitortestCase.xlsx')
    ...
    def interfaceTest(self,num,api_purpose,request_url,request_data,check_point,session):
        url1= 'https://puhui-env.yingu.com/mis/api/misReceiveCustomer/encodeParams'#加密地址
        data = {"jsonParams":request_data,"type":"2"}#加密数据
        ...
        requetsData= requests.post(url=url1, data=data, headers=headers)
        encryptdata = requetsData.text#加密后数据
        # print(encryptdata)
        if session is not None:
            headers['Cookie'] = 'session=' + session
        logindata = {"jsonParams": encryptdata}
        responData = requests.post(url=request_url, data=logindata, headers=headers)#发送的加密后数据
        time.sleep(8)
        ret = responData.text
        # print(ret)
        url2 = 'https://puhui-env.yingu.com/mis/api/misReceiveCustomer/decodeParams'#解密地址
        decodedata = {"jsonParams": ret, "type": "2"}
        result = requests.post(url=url2, data=decodedata, headers=headers)
        resultdata = result.text
        re1 = json.loads(resultdata)#解密数据
        status = re1['rspCode']
        if status == 200:
